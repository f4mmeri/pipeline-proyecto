module controller(
    ...
    output reg       MemReadD,      //para la Hazard Unit
    output reg       BranchD,
    output reg       JumpD,
    output reg       ALUSrcD,
    ...
    always @(*) begin
        case (op)
            7'b0000011: begin // lw
                ...
                MemWriteD   = 1'b0;
                MemReadD    = 1'b1; // Habilitado para detectar Load-Use Hazards
            ...
            7'b0010011: begin // Tipo-I ALU (addi, slli, srli, srai, xori, ori, andi)
                ...
                BranchD     = 1'b0;
                JumpD       = 1'b0;
                ALUOp       = 2'b11; // Determinado por funct3 y funct7 (Variante Tipo-I)
            end
            ...
            7'b1100011: begin // Branches (beq, bne, blt, bge)
                ...
                JumpD       = 1'b0;
                ALUOp       = 2'b01; // SUB (La ALU compara mediante resta interna)
            end
            ...
            default: begin // Estado seguro por defecto (NOP)
                RegWriteD   = 1'b0;
                ImmSrcD     = 2'b00;
                ALUSrcD     = 1'b0;
                MemWriteD   = 1'b0;
    ...
    // DECODIFICADOR DE LA ALU (ALU DECODER)
    always @(*) begin
        case (ALUOp)
            2'b00: ALUControlD = 3'b000; // Suma fija (Suma de direcciones)
            2'b01: ALUControlD = 3'b001; // Resta fija (Comparaciones de Branches)
            
            default: begin // Modos dinámicos 2'b10 (Tipo-R) y 2'b11 (Tipo-I ALU)
                case (funct3)
                    3'b000: if (ALUOp == 2'b10 && funct7b5) 
                                 ALUControlD = 3'b001; // sub (Solo válido en tipo R)
                             else 
                                 ALUControlD = 3'b000; // add / addi
                    3'b001: ALUControlD = 3'b101; // sll / slli (Shift Left Logical)
                    3'b100: ALUControlD = 3'b100; // xor / xori
                    3'b101: if (funct7b5)
                                 ALUControlD = 3'b111; // sra / srai (Shift Right Arithmetic)
                             else
                                 ALUControlD = 3'b110; // srl / srli (Shift Right Logical)
